...
func (r *ChannelRepo) GetByID(ctx context.Context, id uuid.UUID) (*channel.Channel, error) {
	...
	err := r.db.GetContext(ctx, &row,
		`SELECT id, workspace_id, device_id, name, description, visibility, tags, created_at, updated_at FROM channels WHERE id=$1`, id)
...
func (r *ChannelRepo) ListByWorkspace(ctx context.Context, workspaceID uuid.UUID, limit, offset int) ([]*channel.Channel, int64, error) {
	...
	err := r.db.SelectContext(ctx, &rows, `
		SELECT id, workspace_id, device_id, name, description, visibility, tags, created_at, updated_at,
		       COUNT(*) OVER() AS total_count
		FROM channels WHERE workspace_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
...
func (r *ChannelRepo) ListByDevice(ctx context.Context, deviceID uuid.UUID, limit, offset int) ([]*channel.Channel, int64, error) {
	...
	err := r.db.SelectContext(ctx, &rows, `
		SELECT id, workspace_id, device_id, name, description, visibility, tags, created_at, updated_at,
		       COUNT(*) OVER() AS total_count
		FROM channels WHERE device_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
