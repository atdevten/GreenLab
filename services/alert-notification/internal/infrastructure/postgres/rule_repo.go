...
func (r *RuleRepo) Create(ctx context.Context, rule *alert.Rule) error {
	_, err := r.db.NamedExecContext(ctx, `
		INSERT INTO alert_rules (id, channel_id, workspace_id, name, field_name, condition, threshold, severity, message, enabled, cooldown_sec)
		VALUES (:id, :channel_id, :workspace_id, :name, :field_name, :condition, :threshold, :severity, :message, :enabled, :cooldown_sec)`,
		fromRule(rule))
	return err
}
...
func (r *RuleRepo) GetByID(ctx context.Context, id uuid.UUID) (*alert.Rule, error) {
	var row ruleRow
	err := r.db.GetContext(ctx, &row, `SELECT * FROM alert_rules WHERE id=$1`, id)
...
func (r *RuleRepo) ListByChannel(ctx context.Context, channelID uuid.UUID) ([]*alert.Rule, error) {
	var rows []*ruleRow
	err := r.db.SelectContext(ctx, &rows, `SELECT * FROM alert_rules WHERE channel_id=$1 ORDER BY created_at`, channelID)
	if err != nil {
		return nil, err
	}
...
func (r *RuleRepo) ListByWorkspace(ctx context.Context, workspaceID uuid.UUID, limit, offset int) ([]*alert.Rule, int64, error) {
	...
	err := r.db.SelectContext(ctx, &rows,
		`SELECT * FROM alert_rules WHERE workspace_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
